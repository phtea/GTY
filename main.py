...
async def update_users_department_in_db(excel_obj: dict[str, DataFrame]):
    ...
    dep_user_nd_mapping = utils.map_department_nd_to_user_id(dep_analyst_dict, users)
    db_session = db.get_db_session()
...
async def main():
    ...
    to_get_followers    = config.getboolean('Settings', 'to_get_followers', fallback=False)
    use_summaries       = config.getboolean('Settings', 'use_summaries', fallback=False)
    interval_minutes    = config.getint('Settings', 'interval_minutes', fallback=5)
    db_url              = (
...
async def sync_services(queue: str, sync_mode: int, board_id: str,
    ...
    g_tasks_all = await gapi.get_tasks(gapi.Statuses._all)
